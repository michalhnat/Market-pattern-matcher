...
def search_command(args: argparse.Namespace) -> None:
    config = Config(device=args.device, top_k=args.top_k)
    ...
    query_desc = (
        f"Index {args.query_index}"
        if args.query_index is not None
        else args.date
    )
    print(f"\nSearch Results for {config.ticker} (Query: {query_desc})")
    print("-" * 85)
    print(
    ...
    for r in results:
        print(
            f"{r.rank:<5} | {r.distance:<10.4f} | {r.start_date:<12} | "
            f"{r.end_date:<12} | {r.ticker:<8} | {r.window_index:<10}"
        )
    print("-" * 85)
...
def main() -> None:
    ...
    train_parser.add_argument(
        "--data", type=Path, required=True, help="Path to preprocessed data dir"
    )

...
class PatternSearcher:
    ...
    def load_resources(self, index_path: Path = None):
        ...
        self.model = MarketAutoencoder(
            input_len=self.meta_info["window_size"],
            embedding_dim=self.meta_info["embedding_dim"]
        ).to(self.device)
        ...
        if query_index is not None:
             ...
             try:
                 ...
                 if diffs[target_idx].days > 7:
                     logger.warning(f"Requested date {query_date} not found. Closest is {found_date.date()} ({diffs[target_idx].days} days away)")
                 
                 raw_window = self.data_memmap[target_idx]
                 query_vector = self._encode(raw_window)
                 
             except Exception as e:
                 raise ValueError(f"Error resolving date {query_date}: {e}")
        else:
